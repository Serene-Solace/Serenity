...
const StyledWrapper = styled.div<StyledWrapperProps>`
  ...
  .dropdown-container {
    ...
    border-radius: 8px;
    position: relative;
    border: ${({ $customBorder }) => $customBorder || '1px solid rgba(40, 59, 65, 0.7)'};
    cursor: ${({ isDisabled }) => (isDisabled ? 'not-allowed' : 'pointer')};
    background-color: ${({ isDisabled }) => (isDisabled ? '#283b411a' : 'white')};
    display: flex;
    align-items: center;
  }
  ...
  .dropdown-header {
    ...
    display: flex;
    color: ${({ isSelected }) => (isSelected ? 'black' : 'rgba(40, 59, 65, 0.5)')}; /* Change text color based on selection */
  }
  ...
  .dropdown-search {
    ...
    font-size: 14px;
    line-height: 1rem;
    color: ${({ isSelected }) => (isSelected ? 'black' : 'rgba(40, 59, 65, 0.5)')};
    &:focus {
      outline: none;
    }
  }
...
export const CustomDropDown: React.FC<CustomDropDownProps> = ({
  ...
  const filteredOptions = isSearchInput ? options.filter((option) => option.toLowerCase().includes(searchTerm.toLowerCase())) : options
  ...
  return (
    ...
    >
      ...
      <div onClick={toggleDropdown} className='dropdown-container' style={style}>
        {isSearchInput ? (
          <div className='input-placeholder-container-search'>
            <SearchIcon color={isSelected ? 'black' : 'rgba(40, 59, 65, 0.5)'} />
            <input
              type='text'
